...
data CedeModifier = CedeModifier
    ...
    , _cmLookupCache        :: !(HashMap StakeholderId (Maybe ProxySKHeavy))
      ...
      -- invalidation is hard, so we'll just `empty` it every time
      -- something is written, but it optimizes reads. It only holds
      -- db query results unless something comes into cmPskMods, then
      -- we flush.
    }
