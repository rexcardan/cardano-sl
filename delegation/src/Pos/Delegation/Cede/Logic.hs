...
dlgVerifyHeader h = runExceptT $ do
    let bIssuer = h ^. mainHeaderLeaderKey
    ...
    case sig of
        ...
        (BlockSignature _) -> do
            whenJustM (getPskPk bIssuer) $ \psk ->
                throwError $
        ...
        (BlockPSignatureHeavy pSig) -> do
            lastPskM <- dlgLastPsk (addressHash bIssuer)
            let canIssue = maybe False (== psigPsk pSig) lastPskM
            unless canIssue $ throwError $
                sformat ("heavy proxy signature's "%build%" "%
                        ...
                        pSig
        -- We check that the light signature
        (BlockPSignatureLight pSig) -> do
            let pskIPk = pskIssuerPk (psigPsk pSig)
            unless (pskIPk == bIssuer) $ throwError $
